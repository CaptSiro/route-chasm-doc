...
\begin{verbatim}
    ...
    // Query::infer automatically infers the parameter type (TYPE_INT)
...
Default implementation of first and all functions return values from all columns and return one or many User objects.
...
\begin{verbatim}
    // Returns all User class defined columns
    // SELECT id_user, name FROM ...
    User::first(where: Query::infer('id_user = ?', 4));

    // Returns projection columns
    // SELECT name FROM ...
    User::first(projection: ['name'], where: Query::infer('id_user = ?', 4));

    // Manual select query
    // Uses SELECT * FROM ... because projection is not set
